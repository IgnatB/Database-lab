...
def autorisation_window():
    ...
    root = Tk()
    root.title("Log in")
    mainframe = ttk.Frame(root, width=500, height=200)
    mainframe.pack()
    ...
    def checkPerson():
        ...
        f = open('secure.txt', 'r')
        for line in f:
            savedLogin, savedPassword = line.split('>')
    ...
    ttk.Entry(mainframe, width=20, textvariable=logEntry).pack()
    ttk.Label(mainframe, text = 'Password:').pack()
    ttk.Entry(mainframe, width=20, textvariable=pasEntry).pack()
